...
export interface AuthenticatedLibrary {
  user: { id: string; email: string | null };
}

export async function authenticateLibrary(request: NextRequest): Promise<AuthenticatedLibrary> {
  ...
  if (!roles.includes('library')) throw { status: 403, message: 'Forbidden' };
