...
import {
  Search, Home, Users, DollarSign, BarChart3, Sun, Moon, Building2, Package,
} from 'lucide-react';
...
const ManagementLayout = ({ children }: ManagementLayoutProps) => {
  ...
  const navItems = [
    ...
    { href: '/management/infrastructure', label: 'Infrastructure', icon: Package },
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs" style={{ color: 'var(--ch-muted)' }}>
                {user?.email || 'management@campus.edu'}
              </p>
            </div>
          </div>
          <Button
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="text-xs font-bold" style={{ color: 'var(--ch-text)' }}>
                  {user?.name?.split(' ')[0]?.toUpperCase() || 'MANAGEMENT'}
                </span>
              </Button>
            </div>
          </div>
