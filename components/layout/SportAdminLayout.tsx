...
import {
  Home, CalendarDays, Users, Trophy, Sun, Moon, Sparkles,
} from 'lucide-react';
...
interface SportAdminLayoutProps {
  children: ReactNode;
}

const SportAdminLayout = ({ children }: SportAdminLayoutProps) => {
  const { user, logout } = useAuth();
  const { isDark, toggleTheme } = useTheme();
  const pathname = usePathname();

  const navItems = [
    { href: '/sport-admin', label: 'Dashboard', icon: Home },
    { href: '/sport-admin/events', label: 'Events', icon: CalendarDays },
    { href: '/sport-admin/teams', label: 'Teams', icon: Users },
    { href: '/sport-admin/bookings', label: 'Bookings', icon: Trophy },
  ];

  const isActive = (href: string) => {
    if (href === '/sport-admin') return pathname === href;
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="text-xs font-bold" style={{ color: 'var(--ch-text)' }}>
                  {user?.name?.split(' ')[0]?.toUpperCase() || 'SPORT'}
                </span>
              </Button>
            </div>
          </div>
...
export default SportAdminLayout;
