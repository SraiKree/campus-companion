...
import {
  Home, Bus, Map, Users, UserCheck, Wallet, Bell, Sun, Moon,
} from 'lucide-react';
...
const TransportLayout = ({ children }: TransportLayoutProps) => {
  ...
  const navItems = [
    { href: '/transport', label: 'Dashboard', icon: Home },
    { href: '/transport/buses', label: 'Buses', icon: Bus },
    { href: '/transport/routes', label: 'Routes', icon: Map },
    { href: '/transport/drivers', label: 'Drivers', icon: Users },
    { href: '/transport/assignments', label: 'Assignments', icon: UserCheck },
    { href: '/transport/fees', label: 'Fees', icon: Wallet },
    { href: '/transport/notifications', label: 'Notifications', icon: Bell },
  ];

  const isActive = (href: string) => {
    if (href === '/transport') return pathname === href;
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs truncate" style={{ color: 'var(--ch-muted)' }}>
                {user?.email || 'transport@campus.edu'}
              </p>
            </div>
          </div>
          <Button
