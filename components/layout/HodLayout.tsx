...
import {
  Search, Home, Users, GraduationCap, Calendar, FileText, BarChart3, Sun, Moon, BadgeCheck,
} from 'lucide-react';
...
const HodLayout = ({ children }: HodLayoutProps) => {
  ...
  const navItems = [
    ...
    { href: '/hod/attendance', label: 'Attendance', icon: Calendar },
    { href: '/hod/reports', label: 'Reports', icon: BarChart3 },
    { href: '/hod/approvals', label: 'Approvals', icon: FileText },
  ];

  const isActive = (href: string) => {
    if (href === '/hod') return pathname === href;
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 overflow-hidden">
              ...
              <p className="text-xs" style={{ color: 'var(--ch-muted)' }}>
                {user?.department ? `HOD — ${user.department}` : user?.email || 'hod@campus.edu'}
              </p>
            </div>
          </div>
          <Button
          ...
          <div className="h-full px-10 flex items-center justify-between">
            <div className="flex-1 max-w-md">
              <div className="relative">
                ...
                />
                <Input
                  placeholder="Search faculty, students..."
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="text-xs font-bold" style={{ color: 'var(--ch-text)' }}>
                  {user?.name?.split(' ')[0]?.toUpperCase() || 'HOD'}
                </span>
              </Button>
            </div>
          </div>
