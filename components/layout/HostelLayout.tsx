...
import {
  Search, Home, Users, BedDouble, UtensilsCrossed, Wrench, ShieldAlert, UserCheck,
  DoorOpen, Sun, Moon, Building,
} from 'lucide-react';
...
const HostelLayout = ({ children }: HostelLayoutProps) => {
  const { user, logout } = useAuth();
  const { isDark, toggleTheme } = useTheme();
  const pathname = usePathname();

  const navItems = [
    { href: '/hostel', label: 'Dashboard', icon: Home },
    { href: '/hostel/leave', label: 'Leave / Outing', icon: DoorOpen },
    { href: '/hostel/admissions', label: 'Admissions', icon: BedDouble },
    { href: '/hostel/mess', label: 'Mess & Food', icon: UtensilsCrossed },
    { href: '/hostel/maintenance', label: 'Maintenance', icon: Wrench },
    { href: '/hostel/discipline', label: 'Discipline', icon: ShieldAlert },
    { href: '/hostel/visitors', label: 'Visitors', icon: UserCheck },
  ];

  const isActive = (href: string) => {
    if (href === '/hostel') return pathname === href;
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-sm font-bold truncate" style={{ color: 'var(--ch-text)' }}>
                {user?.name || 'Hostel Admin'}
              </p>
              <p className="text-xs" style={{ color: 'var(--ch-muted)' }}>
                {user?.email || 'hostel@campus.edu'}
              </p>
            </div>
          </div>
          <Button
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="text-xs font-bold" style={{ color: 'var(--ch-text)' }}>
                  {user?.name?.split(' ')[0]?.toUpperCase() || 'HOSTEL'}
                </span>
              </Button>
            </div>
          </div>
